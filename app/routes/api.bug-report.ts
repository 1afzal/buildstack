...
const bugReportSchema = z.object({
  ...
  environmentInfo: z
    .object({
      ...
      buildstackVersion: z.string().optional(),
...
function formatIssueBody(data: z.infer<typeof bugReportSchema>): string {
  ...
  if (data.includeEnvironmentInfo && data.environmentInfo) {
    ...
    if (data.environmentInfo.buildstackVersion) {
      body += `- BuildStack: ${data.environmentInfo.buildstackVersion}\n`;
    }
  ...
  body += '---\n*Submitted via BuildStack bug report feature*';
...
export async function action({ request, context: _context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const githubToken = (process.env as any)?.GITHUB_BUG_REPORT_TOKEN || process.env.GITHUB_BUG_REPORT_TOKEN;
    const targetRepo =
      (process.env as any)?.BUG_REPORT_REPO || process.env.BUG_REPORT_REPO || 'buildstack-ai/buildstack';
    ...
    const octokit = new Octokit({
      auth: githubToken,
      userAgent: 'BuildStack-bug-reporter',
    });
