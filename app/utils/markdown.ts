  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === 'html' && node.value && node.value.startsWith('<think>')) {
        ...
        node.value = `<div class="__buildstackThought__">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    div: [
      ...
      [
        'className',
        '__buildstackArtifact__',
        '__buildstackThought__',
        '__buildstackQuickAction',
        '__buildstackSelectedElement__',
      ],

      // ['className', '__buildstackThought__']
    ],
    button: [
      ...
      'name',
      'value',
      ['className', '__buildstackArtifact__', '__buildstackThought__', '__buildstackQuickAction'],
    ],
  },
  strip: [],
};
