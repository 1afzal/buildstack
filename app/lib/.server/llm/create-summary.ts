...
import { extractCurrentContext, extractPropertiesFromMessage, simplifyBuildStackActions } from './utils';
...
export async function createSummary(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifyBuildStackActions(content);
      content = content.replace(/<div class=\\"__buildstackThought__\\">.*?<\/div>/s, '');
