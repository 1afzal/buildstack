...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          buildstack_user_profile: this._safeGetItem('buildstack_user_profile'),
          buildstack_settings: this._safeGetItem('buildstack_settings'),
          buildstack_profile: this._safeGetItem('buildstack_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('buildstack_viewed_features'),
          developer_mode: this._safeGetItem('buildstack_developer_mode'),
        ...
        ui: {
          // Tab configuration
          buildstack_tab_configuration: this._safeGetItem('buildstack_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('buildstack_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('buildstack_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          buildstack_read_logs: this._safeGetItem('buildstack_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('buildstack_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('buildstack_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.buildstack_tab_configuration) {
        try {
          this._safeSetItem('buildstack_tab_configuration', data.ui.buildstack_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'buildstack_acknowledged_debug_issues',
        'buildstack_acknowledged_connection_issue',
        'error_logs',
        'buildstack_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('buildstack_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
