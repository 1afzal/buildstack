  ...
  const style = useMemo(() => {
    ...
    switch (log.level) {
      case 'error':
        return {
          icon: 'i-ph:warning-circle',
          color: 'text-buildstack-elements-button-danger-text',
          bg: 'hover:bg-red-500/10 dark:hover:bg-red-500/20',
          badge: 'text-buildstack-elements-icon-error bg-red-50 dark:bg-red-500/10',
        };
      case 'warning':
        return {
          icon: 'i-ph:warning',
      ...
      case 'debug':
        return {
          ...
          badge: 'text-gray-500 bg-buildstack-elements-background-depth-2',
        };
      default:
        return {
          icon: 'i-ph:info',
  ...
  const renderDetails = (details: any) => {
    if (log.category === 'provider') {
      return (
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 text-xs text-buildstack-elements-textTertiary">
          ...
          {details.prompt && (
            <div className="flex flex-col gap-1">
              <div className="text-xs font-medium text-buildstack-elements-textSecondary">Prompt:</div>
              <pre className="text-xs text-buildstack-elements-textSecondary bg-buildstack-elements-background-depth-2 rounded p-2 whitespace-pre-wrap">
                {details.prompt}
              </pre>
            </div>
          )}
          {details.response && (
            <div className="flex flex-col gap-1">
              <div className="text-xs font-medium text-buildstack-elements-textSecondary">Response:</div>
              <pre className="text-xs text-buildstack-elements-textSecondary bg-buildstack-elements-background-depth-2 rounded p-2 whitespace-pre-wrap">
                {details.response}
              </pre>
            </div>
    ...
    if (log.category === 'api') {
      return (
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 text-xs text-buildstack-elements-textTertiary">
            <span className={details.method === 'GET' ? 'text-buildstack-elements-icon-success' : 'text-blue-500'}>
              {details.method}
            </span>
            <span>•</span>
          ...
          <div className="text-xs text-buildstack-elements-textSecondary break-all">{details.url}</div>
          {details.request && (
            <div className="flex flex-col gap-1">
              <div className="text-xs font-medium text-buildstack-elements-textSecondary">Request:</div>
              <pre className="text-xs text-buildstack-elements-textSecondary bg-buildstack-elements-background-depth-2 rounded p-2 whitespace-pre-wrap">
          ...
          {details.error && (
            <div className="flex flex-col gap-1">
              <div className="text-xs font-medium text-buildstack-elements-icon-error">Error:</div>
    ...
    return (
      <pre className="text-xs text-buildstack-elements-textSecondary bg-buildstack-elements-background-depth-2 rounded whitespace-pre-wrap">
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex flex-col gap-1">
            <div className="text-sm font-medium text-buildstack-elements-textPrimary">{log.message}</div>
            {log.details && (
              <>
                <button
                  onClick={() => setLocalExpanded(!localExpanded)}
                  className="text-xs text-buildstack-elements-textTertiary hover:text-gray-900 dark:hover:text-gray-400 transition-colors"
                >
            ...
            <div className="flex items-center gap-2">
              ...
              {log.category && (
                <div className="px-2 py-0.5 rounded-full text-xs bg-buildstack-elements-background-depth-3 text-buildstack-elements-textTertiary">
                  {log.category}
                </div>
              )}
            </div>
        ...
        {showTimestamp && <time className="shrink-0 text-xs text-buildstack-elements-textTertiary">{timestamp}</time>}
      </div>
    </motion.div>
  );
};
...
export function EventLogsTab() {
  ...
  const exportAsJSON = () => {
    try {
      ...
      a.href = url;
      a.download = `buildstack-event-logs-${new Date().toISOString()}.json`;
  ...
  const exportAsCSV = () => {
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `buildstack-event-logs-${new Date().toISOString()}.csv`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
  ...
  const exportAsPDF = () => {
    try {
      ...
      // Add subtitle with BuildStack
      doc.setFontSize(12);
      doc.setFont('helvetica', 'normal');
      doc.text('BuildStack - AI Development Platform', margin, 45);
      yPos = 70;
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        // Add footer text
        doc.text('Generated by BuildStack', margin, doc.internal.pageSize.getHeight() - 10);
      ...
      // Save the PDF
      doc.save(`buildstack-event-logs-${new Date().toISOString()}.pdf`);
  ...
  const exportAsText = () => {
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `buildstack-event-logs-${new Date().toISOString()}.txt`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
      ...
      <DialogRoot open={isOpen} onOpenChange={handleOpenChange}>
        <button
          ...
          className={classNames(
            ...
            'text-sm text-buildstack-elements-textPrimary',
            'bg-[#FAFAFA] dark:bg-[#0A0A0A]',
            'border border-[#E5E5E5] dark:border-[#1A1A1A]',
            'hover:bg-buildstack-elements-item-backgroundActive',
            'transition-all duration-200',
          )}
        >
          <span className="i-ph:download text-lg text-buildstack-elements-textTertiary group-hover:text-gray-900 transition-colors" />
          Export
        </button>
              ...
              {exportFormats.map((format) => (
                <button
                  ...
                  className={classNames(
                    ...
                    'hover:border-gray-300 dark:hover:border-gray-900/30',
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-xs text-buildstack-elements-textSecondary mt-0.5">
    ...
    <div className="flex h-full flex-col gap-6">
      <div className="flex items-center justify-between">
        <DropdownMenu.Root open={showLevelFilter} onOpenChange={setShowLevelFilter}>
          <DropdownMenu.Trigger asChild>
            ...
            >
              ...
              <span className="i-ph:caret-down text-lg text-buildstack-elements-textTertiary" />
            </button>
          ...
          <DropdownMenu.Portal>
            ...
            >
              {logLevelOptions.map((option) => (
                <DropdownMenu.Item
                  key={option.value}
                  className="group flex items-center px-4 py-2.5 text-sm text-buildstack-elements-textSecondary hover:bg-buildstack-elements-item-backgroundActive cursor-pointer transition-colors"
                  onClick={() => handleLevelFilterChange(option.value)}
                >
                  <div className="mr-3 flex h-5 w-5 items-center justify-center">
                    <div
                      className={classNames(option.icon, 'text-lg group-hover:text-gray-900 transition-colors')}
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            />
            <span className="text-sm text-buildstack-elements-textTertiary">Show Timestamps</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            <Switch
              ...
              className="data-[state=checked]:bg-gray-900"
            />
            <span className="text-sm text-buildstack-elements-textTertiary">24h Time</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            <Switch
              ...
              className="data-[state=checked]:bg-gray-900"
            />
            <span className="text-sm text-buildstack-elements-textTertiary">Auto Expand</span>
          </div>

          <div className="w-px h-4 bg-buildstack-elements-background-depth-3" />
          ...
          >
            <span className="i-ph:arrows-clockwise text-lg text-buildstack-elements-textTertiary group-hover:text-gray-900 transition-colors" />
            Refresh
          </button>
      ...
      <div className="flex flex-col gap-4">
        <div className="relative">
          <input
            ...
            className={classNames(
              ...
              'bg-[#FAFAFA] dark:bg-[#0A0A0A]',
              'border border-[#E5E5E5] dark:border-[#1A1A1A]',
              'text-buildstack-elements-textPrimary placeholder-gray-500 dark:placeholder-gray-400',
              'focus:outline-none focus:ring-2 focus:ring-gray-900/20 focus:border-gray-900',
              'transition-all duration-200',
            )}
          />
          <div className="absolute left-3 top-1/2 -translate-y-1/2">
            <div className="i-ph:magnifying-glass text-lg text-buildstack-elements-textTertiary" />
          </div>
        </div>
        ...
        {filteredLogs.length === 0 ? (
          ...
          >
            <span className="i-ph:clipboard-text text-4xl text-buildstack-elements-textTertiary" />
            <div className="flex flex-col gap-1">
              <h3 className="text-sm font-medium text-buildstack-elements-textPrimary">No Logs Found</h3>
              <p className="text-sm text-buildstack-elements-textTertiary">Try adjusting your search or filters</p>
            </div>
          </motion.div>
        ) : (
          filteredLogs.map((log) => (
