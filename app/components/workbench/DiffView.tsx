...
const FullscreenButton = memo(({ onClick, isFullscreen }: FullscreenButtonProps) => (
  <button
    onClick={onClick}
    className="ml-4 p-1 rounded hover:bg-buildstack-elements-background-depth-3 text-buildstack-elements-textTertiary hover:text-buildstack-elements-textPrimary transition-colors"
...
const FullscreenOverlay = memo(({ isFullscreen, children }: { isFullscreen: boolean; children: React.ReactNode }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-black/50 flex items-center justify-center p-6">
      <div className="w-full h-full max-w-[90vw] max-h-[90vh] bg-buildstack-elements-background-depth-2 rounded-lg border border-buildstack-elements-borderColor shadow-xl overflow-hidden">
        {children}
      </div>
    </div>
  );
...
const lineNumberStyles =
  'w-9 shrink-0 pl-2 py-1 text-left font-mono text-buildstack-elements-textTertiary border-r border-buildstack-elements-borderColor bg-buildstack-elements-background-depth-1';
const lineContentStyles =
  'px-1 py-1 font-mono whitespace-pre flex-1 group-hover:bg-buildstack-elements-background-depth-2 text-buildstack-elements-textPrimary';
...
const changeColorStyles = {
  added: 'text-green-700 dark:text-buildstack-elements-icon-success bg-green-500/10 dark:bg-green-500/20',
  removed: 'text-red-700 dark:text-buildstack-elements-icon-error bg-red-500/10 dark:bg-red-500/20',
  unchanged: 'text-buildstack-elements-textPrimary',
};

const renderContentWarning = (type: 'binary' | 'error') => (
  <div className="h-full flex items-center justify-center p-4">
    <div className="text-center text-buildstack-elements-textTertiary">
      <div className={`i-ph:${type === 'binary' ? 'file-x' : 'warning-circle'} text-4xl text-red-400 mb-2 mx-auto`} />
      <p className="font-medium text-buildstack-elements-textPrimary">
  ...
  ({
    ...
    <div className="h-full flex flex-col items-center justify-center p-4">
      <div className="text-center text-buildstack-elements-textTertiary">
        ...
        <p className="font-medium text-buildstack-elements-textPrimary">Files are identical</p>
        <p className="text-sm mt-1">Both versions match exactly</p>
      </div>
      <div className="mt-4 w-full max-w-2xl bg-buildstack-elements-background-depth-1 rounded-lg border border-buildstack-elements-borderColor overflow-hidden">
        <div className="p-2 text-xs font-bold text-buildstack-elements-textTertiary border-b border-buildstack-elements-borderColor">
          Current Content
        </div>
    ...
    return (
      <div className="flex group min-w-fit">
        ...
        <div className={`${lineContentStyles} ${bgColor}`}>
          <span className="mr-2 text-buildstack-elements-textTertiary">
            {type === 'added' && <span className="text-green-700 dark:text-buildstack-elements-icon-success">+</span>}
            {type === 'removed' && <span className="text-red-700 dark:text-buildstack-elements-icon-error">-</span>}
            {type === 'unchanged' && ' '}
          </span>
          {renderContent()}
        </div>
...
const FileInfo = memo(
  ({
    ...
    return (
      <div className="flex items-center bg-buildstack-elements-background-depth-1 p-2 text-sm text-buildstack-elements-textPrimary shrink-0">
            ...
            <>
              {showStats && (
                <div className="flex items-center gap-1 text-xs">
                  {additions > 0 && (
                    <span className="text-green-700 dark:text-buildstack-elements-icon-success">+{additions}</span>
                  )}
                  {deletions > 0 && (
                    <span className="text-red-700 dark:text-buildstack-elements-icon-error">-{deletions}</span>
                  )}
                </div>
              )}
              <span className="text-yellow-600 dark:text-yellow-400">Modified</span>
              <span className="text-buildstack-elements-textTertiary text-xs">{new Date().toLocaleTimeString()}</span>
            </>
          ) : (
  ...
  if (!highlighter) {
    return (
      <div className="h-full flex items-center justify-center">
        <div className="text-buildstack-elements-textTertiary">Loading diff...</div>
      </div>
    );
  }
...
export const DiffView = memo(({ fileHistory, setFileHistory }: DiffViewProps) => {
  ...
  try {
    ...
    return (
      <div className="flex w-full h-full justify-center items-center bg-buildstack-elements-background-depth-1 text-red-400">
