...
export function TabsWithSlider({
  ...
  return (
    <div className={classNames('relative flex gap-2', className)}>
      ...
      {tabs.map((tab, index) => (
        <button
          ...
          className={classNames(
            ...
            tab.id === activeTab
              ? classNames('text-white shadow-sm shadow-gray-900/20', activeTabClassName)
              : classNames(
                  'bg-buildstack-elements-background-depth-2 dark:bg-buildstack-elements-background-depth-3 text-buildstack-elements-textPrimary dark:text-buildstack-elements-textPrimary-dark hover:bg-buildstack-elements-background-depth-3 dark:hover:bg-buildstack-elements-background-depth-4 border border-buildstack-elements-borderColor dark:border-buildstack-elements-borderColor-dark',
                  tabClassName,
                ),
          )}
        >
      ...
      {/* Animated slider */}
      <motion.div
        className={classNames('absolute bottom-0 left-0 h-10 rounded-lg bg-gray-900 -z-10', sliderClassName)}
        initial={false}
        animate={{
