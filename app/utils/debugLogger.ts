...
class DebugLogger {
  ...
  private async _collectAppInfo(): Promise<AppInfo> {
    ...
    try {
      if (typeof window !== 'undefined') {
        ...
        const workbenchStore = (window as any).__buildstack_workbench_store;
  ...
  private _getCurrentModel(): string {
    try {
      ...
      if (typeof window !== 'undefined') {
        const stored = localStorage.getItem('buildstack_current_model');
  ...
  private _getCurrentProvider(): string {
    try {
      if (typeof window !== 'undefined') {
        const stored = localStorage.getItem('buildstack_current_provider');
  ...
  private _getProjectType(): string {
    try {
      if (typeof window !== 'undefined') {
        const stored = localStorage.getItem('buildstack_project_type');
  ...
  private _getGitInfoFallback(): AppInfo['gitInfo'] {
    try {
      ...
      const stored = localStorage.getItem('buildstack_git_info');
...
export async function downloadDebugLog(filename?: string): Promise<void> {
  try {
    ...
    link.href = url;
    link.download = filename || `buildstack-debug-${new Date().toISOString().split('T')[0]}.txt`;
