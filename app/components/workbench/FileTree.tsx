...
function ContextMenuItem({ onSelect, children }: { onSelect?: () => void; children: ReactNode }) {
  return (
    <ContextMenu.Item
      onSelect={onSelect}
      className="flex items-center gap-2 px-2 py-1.5 outline-0 text-sm text-buildstack-elements-textPrimary cursor-pointer ws-nowrap text-buildstack-elements-item-contentDefault hover:text-buildstack-elements-item-contentActive hover:bg-buildstack-elements-item-backgroundActive rounded-md"
    >
...
function InlineInput({ depth, placeholder, initialValue = '', onSubmit, onCancel }: InlineInputProps) {
  ...
  return (
    <div
      className="flex items-center w-full px-2 bg-buildstack-elements-background-depth-4 border border-buildstack-elements-item-contentAccent py-0.5 text-buildstack-elements-textPrimary"
      style={{ paddingLeft: `${6 + depth * NODE_PADDING_LEFT}px` }}
    >
      <div className="scale-120 shrink-0 i-ph:file-plus text-buildstack-elements-textTertiary" />
      <input
        ref={inputRef}
        type="text"
        className="ml-2 flex-1 bg-transparent border-none outline-none py-0.5 text-sm text-buildstack-elements-textPrimary placeholder:text-buildstack-elements-textTertiary min-w-0"
    ...
    <>
      <ContextMenu.Root>
        <ContextMenu.Trigger>
          <div
            ...
            className={classNames('relative', {
              'bg-buildstack-elements-background-depth-2 border border-dashed border-buildstack-elements-item-contentAccent rounded-md':
                isDragging,
            })}
          >
            {children}
        ...
        <ContextMenu.Portal>
          <ContextMenu.Content
            style={{ zIndex: 998 }}
            className="border border-buildstack-elements-borderColor rounded-md z-context-menu bg-buildstack-elements-background-depth-1 dark:bg-buildstack-elements-background-depth-2 data-[state=open]:animate-in animate-duration-100 data-[state=open]:fade-in-0 data-[state=open]:zoom-in-98 w-56"
          >
            <ContextMenu.Group className="p-1 border-b-px border-solid border-buildstack-elements-borderColor">
            ...
            <ContextMenu.Group className="p-1 border-t-px border-solid border-buildstack-elements-borderColor">
              {!isFolder ? (
                <>
            ...
            <ContextMenu.Group className="p-1 border-t-px border-solid border-buildstack-elements-borderColor">
              <ContextMenuItem onSelect={handleDelete}>
                <div className="flex items-center gap-2 text-buildstack-elements-icon-error">
    ...
    <FileContextMenu onCopyPath={onCopyPath} onCopyRelativePath={onCopyRelativePath} fullPath={folder.fullPath}>
      <NodeButton
        className={classNames('group', {
          'bg-transparent text-buildstack-elements-item-contentDefault hover:text-buildstack-elements-item-contentActive hover:bg-buildstack-elements-item-backgroundActive':
            !selected,
          'bg-buildstack-elements-item-backgroundAccent text-buildstack-elements-item-contentAccent': selected,
        })}
        depth={folder.depth}
      ...
      >
        <div className="flex items-center w-full">
          ...
          {isLocked && (
            <span
              className={classNames('shrink-0', 'i-ph:lock-simple scale-80 text-buildstack-elements-icon-error')}
...
function File({
  ...
  return (
    <FileContextMenu onCopyPath={onCopyPath} onCopyRelativePath={onCopyRelativePath} fullPath={fullPath}>
      <NodeButton
        className={classNames('group', {
          'bg-transparent hover:bg-buildstack-elements-item-backgroundActive text-buildstack-elements-item-contentDefault':
            !selected,
          'bg-buildstack-elements-item-backgroundAccent text-buildstack-elements-item-contentAccent': selected,
        })}
        depth={depth}
        iconClasses={classNames('i-ph:file-duotone scale-98', {
          'group-hover:text-buildstack-elements-item-contentActive': !selected,
        })}
        onClick={onClick}
      >
        <div
          ...
          <div className="flex items-center gap-1">
            {showStats && (
              <div className="flex items-center gap-1 text-xs">
                {additions > 0 && <span className="text-buildstack-elements-icon-success">+{additions}</span>}
                {deletions > 0 && <span className="text-buildstack-elements-icon-error">-{deletions}</span>}
              </div>
            )}
            {locked && (
              <span
