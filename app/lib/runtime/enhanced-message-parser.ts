...
export class EnhancedStreamingMessageParser extends StreamingMessageParser {
  ...
  private _hasDetectedArtifacts(input: string): boolean {
    return input.includes('<buildstackArtifact') || input.includes('</buildstackArtifact>');
  }
  ...
  private _wrapInArtifact(artifactId: string, filePath: string, content: string): string {
    ...
    return `<buildstackArtifact id="${artifactId}" title="${title}" type="bundled">
<buildstackAction type="file" filePath="${filePath}">
${content}
</buildstackAction>
</buildstackArtifact>`;
  }
  ...
  private _wrapInShellAction(content: string, messageId: string): string {
    ...
    return `<buildstackArtifact id="${artifactId}" title="Shell Command" type="shell">
