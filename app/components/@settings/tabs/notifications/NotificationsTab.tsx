...
const NotificationsTab = () => {
  ...
  const getNotificationStyle = (level: string, type?: string) => {
    if (type === 'update') {
      return {
        ...
        color: 'text-buildstack-elements-textSecondary',
        bg: 'hover:bg-buildstack-elements-item-backgroundActive',
      };
    }
  ...
  const renderNotificationDetails = (details: NotificationDetails) => {
    if (details.type === 'update') {
      return (
        <div className="flex flex-col gap-2">
          <p className="text-sm text-buildstack-elements-textSecondary">{details.message}</p>
          <div className="flex flex-col gap-1 text-xs text-buildstack-elements-textTertiary">
    ...
    return details.message ? <p className="text-sm text-buildstack-elements-textSecondary">{details.message}</p> : null;
  };
  ...
  return (
    <div className="flex h-full flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        >
          <span className="i-ph:trash text-lg text-buildstack-elements-textTertiary group-hover:text-gray-900 transition-colors" />
          Clear All
        </button>
      </div>
      ...
      <div className="flex flex-col gap-4">
        {filteredLogs.length === 0 ? (
          ...
          >
            <span className="i-ph:bell-slash text-4xl text-buildstack-elements-textTertiary" />
            <div className="flex flex-col gap-1">
              <h3 className="text-sm font-medium text-buildstack-elements-textPrimary">No Notifications</h3>
              <p className="text-sm text-buildstack-elements-textTertiary">You're all caught up!</p>
            </div>
          </motion.div>
        ) : (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex flex-col gap-1">
                      <h3 className="text-sm font-medium text-buildstack-elements-textPrimary">{log.message}</h3>
                      {log.details && renderNotificationDetails(log.details as NotificationDetails)}
                      <p className="text-xs text-buildstack-elements-textTertiary">
                        Category: {log.category}
                        {log.subCategory ? ` > ${log.subCategory}` : ''}
                      </p>
                    </div>
                  </div>
                  <time className="shrink-0 text-xs text-buildstack-elements-textTertiary">
